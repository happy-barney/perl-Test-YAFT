...
Multiple L<< C<arrange {}>|/arrange {} >> blocks are evaluated in the order of their specification.
...
See also:
L<Test::More::BAIL_OUT|Test::More/BAIL_OUT>,
...
See also:
L<Test::More::diag|Test::More/diag>,
L<Test2::V0::diag|Test2::Tools::Basic/diag>.
...
When called without arguments, the number of tests run is determined
automatically.
...
See also:
L<Test::More::done_testing|Test::More/done_testing>,
L<Test2::V0::done_testing|Test2::Tools::Basic/done_testing>.
...
The C<expect {}> function can be used to compute the expected value for a test assertion
block. Computation is done within the execution context (similar to L</arrange {}>).

It also offers aesthetic clarity when used in conjunction with L</got {}>, as well as
providing a handy way to bypass the stringification effect of a fat comma.
...
See also:
L<Test::More::explain|Test::More/explain>.
...
Unlike L<< C<act {}>|/act {} >>, there is no option to specify dependencies.
...
L<Test2::V0::note|Test2::Tools::Basic/note>.
...
=item Two-value syntax (function override)
...
=item Three-value syntax (object-specific method override):
...
Like L</arrange {}>, when called in a void context, the override is
applied immediately. When used as an assumption parameter, the override
is applied within the assumption's scope.
...
Use L</done_testing> instead if you don't know the test count in advance.

See also:
L</done_testing>,
L<Test::More::plan|Test::More/plan>,
L<Test2::V0::plan|Test2::Tools::Basic/plan>.
...
sub-context for each subtest. This means context-defining blocks
(e.g.: L</act {}>, L</arrange {}>, L</override {}>) defined within
...
the parent context without affecting sibling tests.

See also:
L<Test::More::subtest|Test::More/subtest>,
L<Test2::Tools::Subtest>.
...
Similar to L</expect {}> but for C<throws> assumptions argument.

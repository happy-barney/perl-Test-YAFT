...
Foundation functions are used to build higher level assumptions, expectations, and/or tools.
...
Assumption functions are exported by default.

Every assumption accepts (if any) test message as a first (positional) parameter,
with restof parameters using named approach.

When assumption performs multiple expectations internally, it always reports
...
Utility functions are exported by default.
...
Validity of values follows L<Context::Singleton> rules, so for example
	...
	subtest q (subtest) => sub {
		...
		it q (should ...)
			=> got { ... }
			=> arrange { foo => q (assumption local) }
			;
	};
...
=head2 Foundation Functions

	use Test::YAFT qw (:foundations);

These functions help you write your custom assumptions, expectations, and/or tools.

Foundation functions are not exported by default.

...
subtest q (_decode_override_code ()) => sub {
	...
	my $test_code = sub { q (test-code) };

	assume q (when given a plain coderef returns it)
		=> with_args => [ $test_code ],
		=> expect    =>
			+ expect_shallow ($test_code)
			+ expect_coderef_returns (q (test-code))
		;

	assume q (wrapped coderef returns the scalar value)
		=> with_args => [ q (scalar-value) ],
		=> expect =>
			+ expect_plain_coderef
			+ expect_coderef_returns (q (scalar-value))
		;

	assume q (wrapped coderef returns the arrayref value)
		=> with_args => [ [ q (arrayref-value) ] ],
		=> expect =>
			+ expect_plain_coderef
			+ expect_coderef_returns ([ q (arrayref-value) ])
		;

	assume q (wrapped coderef returns the hashref value)
		=> with_args => [ { key => q (hashref-value) } ],
		=> expect =>
			+ expect_plain_coderef
			+ expect_coderef_returns ({ key => q (hashref-value) })
		;
};
...
subtest q (code override) => sub {
	...
	assume q (override {} is evaluated before got {})
		=> expect   { q (local-override) }
		=> got      { Testing::Override::foo () }
		=> override { q (Testing::Override::foo) => sub { q (local-override) } }
		;
...
subtest q (override with original::method) => sub {
	...
	assume q (original::method works on a blessed instance)
		...
		=> got      { (bless {}, Testing::Override::)->foo }
		=> expect   { q (foo/current-override/instance-override) }
		;
};

subtest q (override inherited method) => sub {
	override { q (Testing::Override::Child::foo) => sub { original::function () . q (/subtest-override) } };

	assume q (original::function resolves to parent implementation)
		=> got    { Testing::Override::Child::->foo }
		=> expect { q (foo/subtest-override) }
		;

	assume q (original::method resolves to parent implementation)
		=> override { q (Testing::Override::Child::foo) => sub { $_[0]->original::method . q (/assumption-override) } }
		=> got      { do { my $obj = bless {}, Testing::Override::Child::; $obj->foo } }
		=> expect   { q (foo/subtest-override/assumption-override) }
		;
};
...
sub expect_coderef_returns {
	state $class = Test::YAFT::test_deep_cmp (
		...
		descend => sub {
			...
			Test::Deep::descend ($got->(@{ $self->{arguments} }), $self->{expect});
		},
	);
...
package Testing::Override::Child {
	use parent -norequire => q (Testing::Override);
}
